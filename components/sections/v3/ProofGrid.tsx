...
export function ProofGrid() {
  return (
    <section
      id="proof"
      className="cosmic-section relative isolate overflow-hidden border-b border-[var(--border)] py-16 sm:py-20"
    >
      ...
      <div className="relative z-10 mx-auto w-full max-w-7xl px-6 sm:px-8 lg:px-10">
        <div className="grid gap-4 lg:grid-cols-[0.65fr_1.35fr]">
          <div>
            <p className="section-eyebrow">04 · Proof</p>
            <h2 className="mt-4 font-display text-4xl font-semibold leading-[1.04] text-[var(--foreground)] sm:text-6xl">
              Signal, compressed.
            </h2>
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
            {cinemaProof.map((item, index) => (
              <article
                key={item.label}
                className="project-card rounded-lg border border-[var(--border)] bg-[var(--panel)] p-5 shadow-[var(--shadow-soft)]"
              >
